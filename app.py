...
# Resume Match and Ranking Script (Web GUI with AI + Deployable + Skill Scoring)
...
def calculate_similarity(text, jd_text):
    vectorizer = TtfidfVectorizer(stop_words='english')
...
def evaluate_skills(resume_text, essential_skills, preferred_skills):
    ...
    total_score = essential_match + preferred_match
    percentage = round((total_score / len(all_skills)) * 100, 2)
    return skill_results, percentage, essential_match, preferred_match
...
def main():
    ...
    with st.sidebar:
        ...
        st.markdown("---")
        essential_input = st.text_area("Essential Skills (comma-separated)",
            "Ruby on Rails, RESTful APIs, PostgreSQL, Backend Ownership, TDD, Agile, Self-driven")
        preferred_input = st.text_area("Preferred Skills (comma-separated)",
        ...
        with st.spinner("Processing resumes..."):
            for resume in resume_files:
                try:
                    ...
                    skill_map, percent_score, essential_hit, preferred_hit = evaluate_skills(
                        text, essential_skills, preferred_skills)

                    results.append({
                        "Candidate": resume.name,
                        "Skill Match %": percent_score,
                        "Match Level": color_match_level(percent_score),
